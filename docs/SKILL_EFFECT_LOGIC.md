...
当前印记以阵营为单位存储在 `BattleState.marks_a` / `BattleState.marks_b`。

- 默认情况下，每个阵营最多同时存在 1 种正面印记和 1 种负面印记。
- 正面印记与负面印记可以并存。
- 新施加的同类印记会覆盖旧的同类印记；同名印记仍按层数叠加。
- 里拉鳐/特性 `吟游之弦` 是例外：它施加的印记不会覆盖已有同类印记，可以让多种正面或多种负面印记同时生效。
- `迟缓印记` 已从战斗原语中移除。

## 状态免疫规则

普通状态施加统一走 `src.engine._monolith._apply_status_stacks()`。

- 火系精灵免疫灼烧。
- 毒系精灵免疫中毒，但不免疫中毒印记。
- 冰系精灵免疫冰冻；`freeze_immune` 特性标记也会免疫冰冻。
- 草系精灵免疫寄生。

这只拦截普通状态层数：`poison_stacks`、`burn_stacks`、`freeze_stacks`、`leech_stacks`。阵营印记仍走印记系统。
